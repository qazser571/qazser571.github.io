document.addEventListener('DOMContentLoaded', () => {
    ...
    const codonTableArea = document.getElementById('codon-table-area');
    ...
    function displayQuestion() {
        if (currentQuestionIndex < quizQuestions.length) {
            ...
            codonTableArea.style.pointerEvents = 'auto';
    ...
    function checkAnswer(selectedCell) {
        ...
        codonTableArea.style.pointerEvents = 'none';
    ...
    function resetQuiz() {
        ...
        codonTableArea.classList.remove('selection-mode');
        codonTableArea.style.pointerEvents = 'auto';

        // setting-area 버튼 제어 코드 제거
        // setButtonState(quizMode1Btn, 'ready');
        // setButtonState(quizMode2Btn, 'ready');
        // setButtonState(quizSelectAllBtn, 'disabled');
        // setButtonState(quizStartBtn, 'disabled');
    ...
    function updateCodonTable() {
        ...
        for (let i = 0; i < firstBases.length; i++) {
            for (let j = 0; j < secondBases.length; j++) {
                ...
                codonTableArea.appendChild(codonBlock);
    ...
    // function updateQuizSelectGauge() { /* ... */ }
    // function updateSettingAreaUI() { /* ... */ }
    // function updateSettingButtonsState() { /* ... */ }

    setTimeout(() => {
        requestAnimationFrame(() => {
            // setting-area 초기 UI 설정 코드 제거
            // control-area-title 초기 UI 설정 (남아있는 부분)
            controlAreaTitles.forEach(title => {
                title.classList.add('control-area-title-active');
                void title.offsetWidth;
            });
            // control-area 버튼 초기 설정 (남아있는 부분)
            updateHideButtonClasses(hideCodonBtn, !showCodons);
            updateHideButtonClasses(hideAminoBtn, !showAminoAcids);
            setButtonState(aminoModeBtn, 'active');
        });
